    ...
    public class WowRootHandler : RootHandlerBase
    {
        ...
        public WowRootHandler(BinaryReader stream, BackgroundWorker worker)
        {
            ...
            while (stream.BaseStream.Position < stream.BaseStream.Length)
            {
                ...
                //Debug.Log("Block: {0} {1} (size {2})", block.ContentFlags, block.LocaleFlags, count);
        ...
        public override void LoadListFile(string path, BackgroundWorker worker)
        {
            ...
            if (!File.Exists(path))
            {
                Debug.Log("WowRootHandler: list file missing!");
                return;
            }

            bool isCsv = Path.GetExtension(path) == ".csv";

            Debug.Log($"WowRootHandler: loading listfile {path}...");
        ...
        public override void Dump()
        {
            ...
            foreach (var fd in RootData.OrderBy(r => r.Key))
            {
                ...
                Debug.Log($"{fd.Key:D7} {fd.Key:X16} {fd.Value.Aggregate(LocaleFlags.None, (a, b) => a | b.LocaleFlags)} {name}");

                foreach (var entry in fd.Value)
                {
                    Debug.Log($"\t{entry.MD5.ToHexString()} - {entry.LocaleFlags} - {entry.ContentFlags}");
                }
            }
        }
    }
