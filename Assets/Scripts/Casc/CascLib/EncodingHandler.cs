                ...
                while ((keysCount = stream.ReadByte()) != 0)
                {
                    ...
                    for (int ki = 0; ki < keysCount; ++ki)
                    {
                        ...
                        if (ki == 0)
                            entry.Key = eKey;
                        //else
                        //    Debug.Log("Multiple encoding keys for MD5 {0}: {1}", md5.ToHexString(), key.ToHexString());

                        //Debug.Log("Encoding {0:D2} {1} {2} {3} {4}", keysCount, aEntries[i].Item1.ToHexString(), aEntries[i].Item2.ToHexString(), md5.ToHexString(), key.ToHexString());
                    }
